...
static int _modbus_tcp_init_win32(void)
{
    ...
    if (WSAStartup(MAKEWORD(2, 2), &wsaData) != 0) {
        Log.error("WSAStartup() returned error code %d\n",
                                              ...
                                              const uint8_t *rsp, int rsp_length)
{
    ...
    if (req[0] != rsp[0] || req[1] != rsp[1]) {
        if (ctx->debug) {
            Log.error("Invalid transaction ID received 0x%X (not 0x%X)\n",
    ...
    if (rsp[2] != 0x0 && rsp[3] != 0x0) {
        if (ctx->debug) {
            Log.error("Invalid protocol ID received 0x%X (not 0x0)\n",
                    (rsp[2] << 8) + rsp[3]);
        }
        errno = EMBBADDATA;
        return -1;
...
static int _modbus_tcp_pi_connect(modbus_t *ctx)
{
    ...
    if (rc != 0) {
        if (ctx->debug) {
            Log.error("Error returned by getaddrinfo: %s\n", gai_strerror(rc));
        }
        errno = ECONNREFUSED;
        return -1;
    }
...
#endif
{
    ...
    if (ctx->debug) {
        Log.info("The client connection from %s is accepted\n",
...
int modbus_tcp_pi_accept(modbus_t *ctx, int *s)
{
    ...
    if (ctx->debug) {
        Log.info("The client connection is accepted.\n");
    }
...
#endif
{
    ...
    if (sigaction(SIGPIPE, &sa, NULL) < 0) {
        ...
        Log.error("Coud not install SIGPIPE handler.\n");
        return NULL;
    }
#endif
    ...
    if (ip != NULL) {
        ...
        if (ret_size == 0) {
            Log.error("The IP string is empty\n");
            modbus_free(ctx);
            errno = EINVAL;
            return NULL;
        }
        ...
        if (ret_size >= dest_size) {
            Log.error("The IP string has been truncated\n");
            modbus_free(ctx);
            errno = EINVAL;
            return NULL;
        }
...
modbus_t* modbus_new_tcp_pi(const char *node, const char *service)
{
    ...
    if (node == NULL) {
        ...
        if (ret_size == 0) {
            Log.error("The node string is empty\n");
            modbus_free(ctx);
            errno = EINVAL;
            return NULL;
        }
        ...
        if (ret_size >= dest_size) {
            Log.error("The node string has been truncated\n");
            modbus_free(ctx);
            errno = EINVAL;
            return NULL;
        }
    ...
    if (ret_size == 0) {
        Log.error("The service string is empty\n");
        modbus_free(ctx);
        errno = EINVAL;
        return NULL;
    }
    ...
    if (ret_size >= dest_size) {
        Log.error("The service string has been truncated\n");
        modbus_free(ctx);
        errno = EINVAL;
        return NULL;
    }
